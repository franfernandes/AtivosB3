        ...
        if ativo.limiar_compra and cotacao_atual <= ativo.limiar_compra:
            for usuario in ativo.usuarios_monitorando.all():
                ...
                try:
                    ...
                    logger.info(
                        f"Email de compra enviado para {email_destino} sobre o ativo {ativo.codigo}"
                    )
                except Exception as e:
                    logger.error(
                        f"Erro ao enviar email de compra para {email_destino}: {e}"
                    )

        if emails_enviados:
            logger.info(
                f"Emails enviados para {', '.join(emails_enviados)} sobre o ativo {ativo.codigo}"
            )
        else:
